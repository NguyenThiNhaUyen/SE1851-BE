...
// => Sử dụng UrgentDonorDetailDTO, dùng service:
    ...
    public void register(Long userId, DonorReadinessLevel level, UrgentDonorRegistrationDTO dto) {
        ...
        // ✅ 2. Lấy UserProfile (nếu cần cập nhật tọa độ hoặc bloodType)
        UserProfile profile = user.getUserProfile();
        if (profile == null) {
            throw new IllegalStateException("Người dùng chưa có hồ sơ cá nhân. Không thể đăng ký hiến máu khẩn cấp.");
        }

        // ✅ 3. Cập nhật tọa độ và nhóm máu cho hồ sơ (nếu chưa có)
        ...
        userProfileRepository.save(profile); // Lưu lại hồ sơ

        // ✅ 4. Kiểm tra xem đã từng đăng ký khẩn cấp chưa
        ...
        if (registry == null) {
            ...
            registry.setRegisteredAt(LocalDateTime.now()); // Thời gian đăng ký mới
        }

        // ✅ 5. Gán thông tin cho bản ghi khẩn cấp
        registry.setBloodType(profile.getBloodType()); // Dùng từ profile sau khi đã cập nhật
        registry.setReadinessLevel(level); // readiness level: EMERGENCY_NOW, etc.
        registry.setLatitude(dto.getLatitude());
        registry.setLongitude(dto.getLongitude());
        registry.setIsAvailable(true);  // Có sẵn để gọi
        registry.setIsVerified(false);  // Cần xác minh
        registry.setLeftGroupAt(null);  // Reset nếu trước đó đã rời nhóm
